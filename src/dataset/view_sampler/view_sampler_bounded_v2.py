...
class ViewSamplerBoundedV2Cfg:
    ...
    extra_views_sampling_strategy: Optional[Literal["random", "farthest_point", "equal", "kmeans"]] = "random"
    ...
    def sample(
        ...
        if total_num_views > 2:
            ...
            if self.cfg.extra_views_sampling_strategy == 'random':
                ...
                #         (num_extra_views,),
                #     ).tolist()
                extra_views = torch.randperm(index_context_right - (index_context_left + 1))[:num_extra_views]
                extra_views = (extra_views + (index_context_left + 1)).tolist()
            ...
            elif self.cfg.extra_views_sampling_strategy == 'kmeans':
                context_bounded_index = torch.arange(index_context_left, index_context_right + 1)
                candidate_views_position = extrinsics[context_bounded_index, :3, 3]
                candidate_view_direction = extrinsics[context_bounded_index, :3, 2]
                candidate_views_position = torch.cat(
                    [candidate_views_position, candidate_view_direction], dim=-1
                ).cpu().numpy()
                input_frame_idx = []
                cluster_centers = KMeans(n_clusters=total_num_views, random_state=0, n_init='auto').fit(candidate_views_position).cluster_centers_
                for center in cluster_centers:
                    dists = np.linalg.norm(candidate_views_position - center, axis=1)
                    input_frame_idx.append(context_bounded_index[np.argmin(dists).item()].item())
                index_context = sorted(input_frame_idx)
                index_context_left = index_context[0]
                index_context_right = index_context[-1]
                extra_views = index_context[1:-1]
        ...
        context_indices = torch.tensor([index_context_left, *extra_views, index_context_right], device=device)
        context_indices = context_indices.sort()[0]
        index_target = index_target.sort()[0]        
